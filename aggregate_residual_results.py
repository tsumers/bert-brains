...
for dataset in ['slumlordreach','black']:
	...
	#Black 
		...
		save_dir=d+"results/black/encoding_residuals/"
	write_dir=d+"results/"+dataset+"/encoding_residuals/" 
	for which in [0,1]:
		for sub in subs:
			num_idx=0
			buffer=[]
			while num_idx<len(numbers)-1:
				...
				buffer.append(np.load(save_dir+sub+"_partial_headwise_result_"+str(lower_p)+"_"+str(upper_p)+"_"+str(which)+".npy"))
				num_idx+=1
			aggregate=np.concatenate(buffer,axis=0) 
			
			print(write_dir+sub+"_"+str(which)+"_parcelwise_results.npy",aggregate.shape) 
			np.save(write_dir+sub+"_"+str(which)+"_parcelwise_results.npy",aggregate) 
