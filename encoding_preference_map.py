...
data_dir=sys.argv[3]
save_dir=sys.argv[4] 
lower_p=int(sys.argv[5])
upper_p=int(sys.argv[6])
...
def process(i): 
	...
	#test_performances=np.zeros((12,3))
	...
	for train_index,test_index in skf.split(X):
		...
		encoding_model_weights.append(original_coef) 
		"""
		for lnum in range(12):
			lnum_coef=original_coef.copy() 
			for zero_layer in range(12):
				if zero_layer!=lnum:
					for d in range(4):
			...
			test_performances[lnum,test_idx]=pearsonr(y_hat[:,0],y_test[:,0])[0]
			#print(lnum,test_idx,test_performances[lnum,test_idx])
		"""
		y_hat=model.predict(X_test)
		test_performances[test_idx]=pearsonr(y_hat[:,0],y_test[:,0])[0]
		#print(test_performances[test_idx])
		test_idx+=1
		#print(test_performances[:,test_idx-1]) 

	test_performances[np.isnan(test_performances)]=0.0
	encoding_model_weights=np.asarray(encoding_model_weights)
	#return np.mean(test_performances,axis=1)
	#print(np.mean(test_performances),encoding_model_weights.shape)
	print(str(parcel_number)+"======================="+str(np.mean(test_performances))+"============")
	return [np.mean(test_performances),np.mean(encoding_model_weights,axis=0)]

raw_results=[] 
save_weights=[]
#roi_mapping=np.load('/jukebox/griffiths/bert-brains/black_data/Parcel2ROI.npy')
for i in range(lower_p,upper_p):
	results_parcel=process(i)
	raw_results.append(results_parcel[0])
	save_weights.append(results_parcel[1])
raw_results=np.asarray(raw_results)
save_weights=np.asarray(save_weights)
np.save(save_dir+sub+"_partial_parcelwise_results_"+str(lower_p)+"_"+str(upper_p)+".npy",raw_results)  
np.save(save_dir+sub+"_partial_encoding_weights_"+str(lower_p)+"_"+str(upper_p)+".npy",save_weights) 
