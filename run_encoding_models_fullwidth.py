...
with open("joblist.txt","w") as f:
	for dataset in ['black','slumlordreach']:
		...
		if dataset=='slumlordreach':
			...
			save_dirs=['/jukebox/griffiths/bert-brains/results/slumlordreach/encoding-fullwidth-holdout_'+str(i)+"/" for i in range(-1,13)]
		else:
			...
			save_dirs=['/jukebox/griffiths/bert-brains/results/black/encoding-fullwidth-holdout_'+str(i)+"/" for i in range(-1,13)]
		...
		for sub in subs: 
			for l_idx,save_dir in enumerate(save_dirs):
				if not(os.path.exists(save_dir+sub+"_parcelwise_results.npy")):
					layer_name=layer_names[l_idx] 
					f.write("python encoding_model_fullwidth.py "+sub+" "+str(layer_name)+" "+data_dir+" "+save_dir+"\n")
	f.close()   
