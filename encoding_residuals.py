...
def model_training(X,y,is_primary_features,train_index,test_index):
	tuned_alphas=[]
...
def process(i): 
	...
	if which:
		...
		is_primary_features2=is_primary2[0,:].astype('bool')
	else:
		X=features2
		X2=features
		is_primary_features=is_primary2[0,:].astype('bool')
		is_primary_features2=is_primary[0,:].astype('bool') 
	...
	for train_index,test_index in skf.split(X):
		...
		ytr,yte=model_training(X2,y,is_primary_features2,train_index,test_index)
		y_train=y_train-ytr 
		y_test=y_test-yte 
...
np.save(save_dir+sub+"_partial_headwise_result_"+str(lower_p)+"_"+str(upper_p)+"_"+str(which)+".npy",raw_results_total) 
