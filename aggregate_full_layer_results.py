...
for dataset in ['slumlordreach']:
	...
	if dataset=='slumlordreach': 
		...
		save_dir=d+"results/slumlordreach/encoding_full/"
	...
	#Black 
		...
		save_dir=d+"results/black/encoding_full/"
	...
	for sub in subs:
		if not(os.path.exists(save_dir+sub+"_parcelwise_results.npy")):
			...
			while num_idx<len(numbers)-1:
				...
				buffer.append(np.load(save_dir+sub+"_partial_total_result_"+str(lower_p)+"_"+str(upper_p)+".npy"))
				num_idx+=1
			aggregate=np.concatenate(buffer,axis=0)
			print(save_dir+sub+"_parcelwise_results.npy",aggregate.shape)
			np.save(save_dir+sub+"_parcelwise_results.npy",aggregate)
