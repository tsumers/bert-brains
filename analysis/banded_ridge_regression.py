...
for train_index,test_index in skf_outer.split(X):
	...
	model.fit([X0_train,X1_train,X2_train,X_train],Y_train) 
	...
	Y_pred=model.predict([X0_test,X1_test,X2_test,X_test],split=True)
	#print(Y_pred.shape)
	for split_idx in range(4): 
		for p in range(1000): 
			performances[p,test_idx,split_idx]=pearsonr(Y_pred[split_idx][:,p],Y_test[:,p])[0]
